...
(function () {
  ...
  Array.prototype.forEach.call(document.querySelectorAll('.demo'), function (demo) {
    ...
    if (aisW) {
      var aisText = aisW.querySelector('.ais-text'), aisRead = aisW.querySelector('.ais-read'), aisWords = [], aisPos = 0, aisTimer = null, aisT0 = 0, aisRan = false;
      function aisTick() { if (aisPos >= aisWords.length) { aisFinish(); return; } aisText.textContent += (aisPos ? ' ' : '') + aisWords[aisPos++]; aisTimer = setTimeout(aisTick, 70); }
      function aisFinish() { aisTimer = null; aisW.classList.remove('is-streaming', 'is-stopped'); aisText.classList.remove('is-streaming'); aisRead.textContent = ga('done').replace('%d', aisWords.length).replace('%s', ((performance.now() - aisT0) / 1000).toFixed(1)); }
      function aisStart() { aisRan = true; clearTimeout(aisTimer); aisWords = ga(demo.classList.contains('on-draft') ? 'dtext' : 'text').split(' '); aisPos = 0; aisText.textContent = ''; aisT0 = performance.now(); aisW.classList.add('is-streaming'); aisW.classList.remove('is-stopped'); aisText.classList.add('is-streaming'); aisRead.textContent = demo.classList.contains('on-int') ? '' : ga('noint'); aisTick(); }
      function aisStop() { if (!aisTimer) return; clearTimeout(aisTimer); aisTimer = null; aisW.classList.remove('is-streaming'); aisW.classList.add('is-stopped'); aisText.classList.remove('is-streaming'); aisRead.textContent = ga('stopped').replace('%d', aisPos); }
    ...
    var mkoW = demo.querySelector('.mko-wrap');
    if (mkoW) {
      var mkoRead = mkoW.querySelector('.mko-read'), mkoHue = 0, mkoHues = ga('hues').split('|');
      function mkoUpd() { var bad = demo.classList.contains('on-gray') && !demo.classList.contains('on-label'); mkoRead.textContent = (bad ? ga('bad') : ga('ok')) + ' · ' + ga('hue').replace('%s', mkoHues[mkoHue]); }
      mkoW.querySelector('.mko-mark').addEventListener('click', function () { mkoHue = (mkoHue + 1) % 3; mkoW.setAttribute('data-hue', String(mkoHue)); mkoUpd(); });
      aiOn(mkoUpd); demo._replay = function () {};
    }
    // Motion: streaming text
    var stxW = demo.querySelector('.stx-wrap');
    if (stxW) {
      var stxText = stxW.querySelector('.stx-text'), stxBelow = stxW.querySelector('.stx-below'), stxRead = stxW.querySelector('.stx-read'), stxTimer = null, stxRan = false;
      aiOn(function () { if (aiOut) aiOut.textContent = ga('ch').split('|')[Number(demo.getAttribute('data-chunk')) || 0]; });
      function stxRun() {
